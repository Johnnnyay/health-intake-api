...
const { extractJson } = require('./extract');
...
async function fetchSpec() {
  ...
  /* getFile now throws when the store is unreachable rather than returning null, so a
     revoked token cannot be mistaken for a missing file. Serving a stale spec is still
     better than failing; generating with NO spec never is. */
  let text = null;
  try {
    text = await getPrivateFile('report-spec.md');
  } catch (e) {
    if (_specCache.text) return _specCache.text;
    throw e;
  }
  if (!text) {
...
async function fetchNutrients() {
  ...
  let text = null;
  try {
    text = await getPrivateFile('nutrient-reference.json');
  } catch (e) {
    /* Stale beats nothing. But nothing beats a guess: with no verified values at all,
       the model invents ULs, which is the failure that told a client magnesium has no
       ceiling. Fail the generation instead. */
    if (_nutCache.text) return _nutCache.text;
    throw e;
  }
  if (!text) return _nutCache.text || null;
...
function formatForm(form) {
  ...
  /* What they most want to improve, in the order they ranked it. This is what the report is
     ordered by (report-spec.md, STEP 1b). It is what they care about, which is not the same as
     what is most wrong: someone with sleep and gut signals may have come to lose weight. */
  if (Array.isArray(form.priorities) && form.priorities.length) {
    lines.push('--- Their priorities: what they most want to improve, in the order they ranked it ---');
    lines.push('This sets the ORDER of the report. It is what they care about, not necessarily what is most wrong.');
    form.priorities.slice().sort((a, b) => (a.rank || 9) - (b.rank || 9)).forEach(pr => {
      const name = pr.id === 'other' ? `Something else: ${pr.other || 'not specified'}` : (areaLabel(pr.id, 'en') || pr.id);
      lines.push(`${pr.rank || '?'}. ${name}` + (pr.words ? `. In their words, better looks like: "${String(pr.words).replace(/\s+/g, ' ').slice(0, 300)}"` : ''));
    });
    lines.push('');
  }
  ...
  const KNOWN = new Set(['name', 'email', 'dob', 'gender', 'height', 'weight', 'weightKg',
    ...
    'diagnosedNote', 'motivation', 'priorities', 'medication', 'dietFrequency', 'smoking', 'formVersion', 'replaces']);
