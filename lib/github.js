...
function ghRequest(method, path, body, repo) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const req = https.request(options, (res) => {
      let data = '';
      res.setEncoding('utf8');   // file contents come back as JSON with raw UTF-8; see report.js
...
async function deleteFile(path, message) {
  const head = await ghRequest('GET', path);
  if (head.status === 404) return false;   // nothing to delete, not an error
  if (head.status !== 200 || !head.data.sha) {
    const e = new Error(`could not read ${path} before deleting: GitHub returned ${head.status}`);
    e.storeUnreachable = true;
    throw e;
  }
  const res = await ghRequest('DELETE', path, { message, sha: head.data.sha, branch: 'main' });
  if (res.status !== 200) {
    throw new Error(`delete of ${path} failed: GitHub returned ${res.status}`);
  }
  return true;
}
...
function cors(req, res) {
  ...
  res.setHeader('Access-Control-Allow-Headers', 'Content-Type,Authorization,x-intake-secret,x-admin-key,x-edit-key');
}

module.exports = { ghRequest, getFile, pushFile, deleteFile, getIndex, cors, fromKnownOrigin, REPORTS_REPO };
