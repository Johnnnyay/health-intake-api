...
function buildHTML(r, form, filename, assessmentDate, locale, available, opts) {
  /* The product list is locked until an IBO unlocks it on the report itself (api/unlock.js). */
  const unlocked = !!(opts && opts.unlocked);
...
<div class="hero">
  ...
  <button class="ibo-btn${unlocked ? ' on' : ''}" id="ibo-btn" onclick="iboToggle()" title="${T.iboLabel}" aria-label="${T.iboLabel}">${unlocked ? T.iboUnlocked : '···'}</button>
...
<div class="page-body">
  ...
  ${(r.products && r.products.length && r.calcUrl) ? (unlocked ? `
...
/* Partner control. Locked: the passcode unlocks the product list for this report.
   Unlocked: the passcode locks it again. The code is checked on the server. */
var IBO_T = ${JSON.stringify({ prompt: T.iboPrompt, wrong: T.iboWrong, lockQ: T.iboLockQ, fail: T.iboFail })};
var IBO_UNLOCKED = ${unlocked ? 'true' : 'false'};
function iboToggle(){
  var code = prompt(IBO_T.prompt);
  if (code === null) return;
  if (IBO_UNLOCKED && !confirm(IBO_T.lockQ)) return;
  var btn = document.getElementById('ibo-btn'); btn.disabled = true;
  fetch('/api/unlock', { method: 'POST', headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ rid: ${JSON.stringify(rid)}, code: String(code).trim(), lock: IBO_UNLOCKED }) })
    .then(function(r){ return r.json().then(function(j){ return { ok: r.ok, status: r.status, j: j }; }); })
    .then(function(x){
      if (x.ok) { location.reload(); return; }
      btn.disabled = false;
      alert(x.status === 401 ? IBO_T.wrong : IBO_T.fail);
    })
    .catch(function(){ btn.disabled = false; alert(IBO_T.fail); });
}
</script>
</body>
</html>`;
