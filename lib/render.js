...
const { CHROME, STAGES, LOCALES, LOCALE_NAME, LOCALE_SHORT, CANONICAL, stageKey } = require('./i18n');
...
function buildHTML(r, form, filename, assessmentDate, locale, available) {
  ...
  const productCard = ({ p, i }) => {
    const rat = p.rationale, not = p.note;
    const d = p.dosing || {};
    return `
    <div class="product-card">
      <div class="product-head">
        ...
        ${d.servingsPerDay ? `<span class="product-dose">${d.servingsPerDay}${d.unit ? ' ' + d.unit : ''}/day</span>` : ''}
      </div>
  ...
  /* Any product with dosing appears here, whether or not it has a nutrient table.
     Filtering on the table dropped plant protein -- the item whose amount mattered
     most in the report it was written for, since protein is dosed per kilogram of
     body weight rather than against a UL. */
  const dosingRows = (r.products || []).filter(p => p.dosing && p.dosing.servingsPerDay);
  const dosingHTML = !dosingRows.length ? '' : `
  <div class="card">
    <div class="card-title">${T.dosing}</div>
    <p class="card-sub">${T.dosingSub}</p>
    <div class="dose-defs">
      <div class="dose-def"><span class="dose-def-t">${T.dLabel}</span><span>${T.dLabelSub}</span></div>
      <div class="dose-def"><span class="dose-def-t">${T.dAI}</span><span>${T.dAISub}</span></div>
      <div class="dose-def"><span class="dose-def-t">${T.dUL}</span><span>${T.dULSub}</span></div>
    </div>
    ${dosingRows.map(p => {
      const d = p.dosing;
      return `
      <div class="dose-block">
        <div class="dose-head">
          <span class="dose-name">${p.name}</span>
          <span class="dose-amt">${d.servingsPerDay} ${d.unit || ''}/day</span>
          ${d.bottlesPerMonth ? `<span class="dose-month">${T.dMonth}: ${d.bottlesPerMonth} ${T.dBottles}</span>` : ''}
        </div>
        ${d.when ? `<div class="dose-line"><b>${T.dWhen}</b> ${d.when}</div>` : ''}
        ${d.why ? `<div class="dose-line"><b>${T.dWhy}</b> ${d.why}</div>` : ''}
        ${(d.nutrients || []).length ? `<table class="dose-table">
          <tr><th></th><th>${T.dLabel}</th><th>/day</th><th>${T.dOfUL}</th></tr>
          ${(d.nutrients || []).map(n => `<tr${n.limiting ? ' class="dose-lim"' : ''}>
            <td>${n.name}${n.limiting ? ` <span class="dose-tag">${T.dLimiting}</span>` : ''}</td>
            <td>${n.perServing}</td><td>${n.total}</td><td>${n.pctUL}%</td></tr>`).join('')}
        </table>` : ''}
        ${d.tolerability ? `<div class="dose-flush"><b>${T.dFlush}</b> ${d.tolerability}</div>` : ''}
        ${(d.nutrients || []).some(n => n.source) ? `<div class="dose-src">${T.dSource}: ${
          [...new Map((d.nutrients || []).filter(n => n.source).map(n => [n.source.url, n.source])).values()]
            .map(sc => `<a href="${sc.url}" target="_blank">${sc.name}</a>${sc.updated ? ` (${sc.updated})` : ''}`).join(' · ')
        }</div>` : ''}
      </div>`;
    }).join('')}
    <p class="dose-cite">${T.dCitation}</p>
  </div>`;


  /* Two routes for every action. "Lifestyle only" as an option was never real: a
     person disciplined enough to do it would not have arrived with these symptoms.
     What is real is the choice between getting something from food or from a
     supplement, and that choice needs the quantities on both sides to be honest. */
  const actions = r.actions || [];
  const planHTML = !actions.length ? '' : `
  <div class="card">
    <div class="card-title">${T.plan}</div>
    <p class="card-sub">${T.planSub}</p>
    ${actions.map((ac, i) => `
      <div class="action">
        <div class="action-head"><span class="action-num">${i + 1}</span><span class="action-do">${ac.action}</span></div>
        <div class="route-grid">
          ${ac.food ? `<div class="route route-food"><div class="route-label">${T.fromFood}</div><div>${ac.food}</div></div>` : ''}
          ${ac.supplement ? `<div class="route route-supp"><div class="route-label">${T.fromSupp}</div><div>${ac.supplement}</div></div>` : ''}
        </div>
        ${ac.track ? `<div class="action-track"><b>${T.trackThis}</b> ${ac.track}</div>` : ''}
      </div>`).join('')}
...
<div class="page-body">
  ...
  <div class="card">
    <div class="card-title">${T.stagesTitle}</div>
    <p class="card-sub">${T.stagesSub}</p>
    ${tierHTML}
    ${stagesPlanHTML}
  </div>
