...
function buildHTML(r, form, filename, assessmentDate, locale, available) {
  ...
  const withTerms = (text) => String(text || '').replace(/\[\[([A-Z%]+)\]\]/g, (m, key) => {
    ...
    return `<span class="term" tabindex="0">${key}` +
  ...
  /* One place that explains the vocabulary properly, listing only the terms this
     report actually used. The inline tooltips are for the reader who stops mid
     sentence; this is for the one who wants it laid out. */
  const usedTerms = [...new Set(
    JSON.stringify(r).match(/\[\[([A-Z%]+)\]\]/g) || []
  )].map(m => m.replace(/\[|\]/g, '')).filter(k => GLOSSARY[k]);
  const refCardHTML = !usedTerms.length ? '' : `
  <div class="card-rule"></div>
...
<style>
  ...
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  .term{position:relative;cursor:help;outline:none;white-space:nowrap;
    border-bottom:1px dashed var(--green);padding-bottom:1px;font-weight:600;color:var(--navy)}
  .term:hover,.term:focus{border-bottom-style:solid}
  /* Opens ABOVE the term. Below covered the line the reader was about to read next,
     which is the one thing a definition must not do. */
  .term-pop{display:none;position:absolute;left:0;bottom:calc(100% + 8px);z-index:400;
    width:min(300px,78vw);white-space:normal;
    background:var(--navy);color:rgba(255,255,255,.92);border-radius:10px;padding:11px 13px;
    box-shadow:0 10px 28px rgba(0,0,0,.22);font-size:12px;line-height:1.6;
    font-weight:400;text-transform:none;letter-spacing:0}
  .term-pop::after{content:"";position:absolute;left:14px;top:100%;border:6px solid transparent;
    border-top-color:var(--navy)}
  .term.term-flip .term-pop::after{left:auto;right:14px}
  .term:hover .term-pop,.term:focus .term-pop,.term:focus-within .term-pop{display:block}
  .term-pop a{color:#fff;text-decoration:underline}
  .term-pop-t{display:block;font-weight:700;color:#fff;margin-bottom:4px;font-size:12.5px}
