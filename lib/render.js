...
function buildHTML(r, form, filename, assessmentDate, locale, available, opts) {
  /* opts (see api/report.js): gate = the report is locked, render only a placeholder page;
     ibo = a signed-in partner is looking; unlocked = the product list is open; reportOpen = the
     report itself is open to the client. */
  opts = opts || {};
  const gate = !!opts.gate, ibo = !!opts.ibo, unlocked = !!opts.unlocked, reportOpen = !!opts.reportOpen;
  ...
  /* Calendar age as of the ASSESSMENT, not as of whenever the page is opened.
     Two bugs lived in the one line this replaces. Dividing elapsed milliseconds by
     365.25 days is wrong for roughly one birthday in sixteen, always by a whole
     year; and 'YYYY-MM-DD' parses as UTC midnight, so a negative-offset timezone
     pushes it back another day. On top of that the age moved every time the report
     was reopened, so a metabolic-age gap drifted after the fact for people whose
     inputs were correct all along. */
  const ymd = (v) => { const m = /^(\d{4})-(\d{2})-(\d{2})/.exec(String(v || '')); return m ? [+m[1], +m[2], +m[3]] : null; };
  const ageOn = (dob, on) => {
    const b = ymd(dob); if (!b) return null;
    const n = new Date();
    const o = ymd(on) || [n.getFullYear(), n.getMonth() + 1, n.getDate()];
    let a = o[0] - b[0];
    if (o[1] < b[1] || (o[1] === b[1] && o[2] < b[2])) a--;
    return a >= 0 && a < 130 ? a : null;
  };
  const ageNum = ageOn(form.dob, assessmentDate);
  ...
  /* Without a usable date of birth there is no gap to state. Printing one anyway
     produced "NaN years older", which is worse than leaving the row out. */
  if (form.metabolicAge && ageNum !== null && !isNaN(parseInt(form.metabolicAge, 10))) {
    const diff = parseInt(form.metabolicAge, 10) - ageNum;
  ...
  /* Always all four, always smallest to largest, whether or not the analysis
     happened to name them. The point of the card is not to gloss the words that
     appear above it: it is that %DV is the only one of the four most people have
     ever seen, and it is the one they should not be measuring themselves against.
     A reader cannot draw that contrast from whichever subset a given report used. */
  const REF_ORDER = ['DV', 'RDA', 'AI', 'UL'];
  const refCardHTML = `
  <div class="card-rule"></div>
  ...
  <div class="ref-grid">
    ${REF_ORDER.map((k, i) => {
      ...
      return `<div class="ref-item">
        <div class="ref-term"><span class="ref-step">${i + 1}</span>${title}</div>
    ...
    ${expectations.map(e => `
      <div class="expect-row">
        ...
        <div class="expect-body">
          <div class="expect-finding">${e.headline || e.finding}</div>
  ...
  /* What the client asked us to focus on, shown back to them in the order they ranked it. */
  const escP = (x) => String(x == null ? '' : x).replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/"/g, '&quot;');
  const prioHTML = (Array.isArray(form.priorities) && form.priorities.length) ? `
    <div class="prio-block">
      <div class="prio-title">${T.prioTitle}</div>
      <ol class="prio-list">${form.priorities.slice().sort((a, b) => (a.rank || 9) - (b.rank || 9)).map((pr, i) => {
        const name = pr.id === 'other' ? (pr.other || '') : (areaLabel(pr.id, locale) || pr.id);
        return `<li class="prio-item"><span class="prio-n">${i + 1}</span><span><b>${escP(name)}</b>${pr.words ? `<span class="prio-words">${escP(pr.words)}</span>` : ''}</span></li>`;
      }).join('')}</ol>
    </div>` : '';
  const skW = [96, 88, 92, 70, 84];
  const skeleton = [0, 1, 2, 3].map(() => `<div class="card"><div class="sk sk-h"></div>${skW.map(w => `<div class="sk" style="width:${w}%"></div>`).join('')}</div>`).join('');
...
<style>
  ...
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  .ref-term{font-size:12.5px;font-weight:700;color:var(--navy);margin-bottom:4px;display:flex;align-items:center;gap:7px}
  .ref-step{display:inline-grid;place-items:center;width:17px;height:17px;border-radius:50%;
    background:var(--green);color:#fff;font-size:10px;font-weight:700;flex-shrink:0}
  ...
  .hero-tools{position:absolute;top:14px;right:18px;z-index:300;display:flex;align-items:center;gap:8px}
  .lang-wrap{position:relative}
  .ibo-btn{display:inline-flex;align-items:center;background:rgba(255,255,255,0.1);border:1px solid rgba(255,255,255,0.16);
    color:rgba(255,255,255,0.85);font-family:inherit;font-size:12px;font-weight:700;letter-spacing:.06em;padding:6px 11px;
    border-radius:18px;cursor:pointer;white-space:nowrap;transition:background .18s}
  .ibo-btn:hover{background:rgba(255,255,255,0.2)}
  .ibo-btn.on{background:rgba(255,255,255,0.22);letter-spacing:0}
  ...
  @media(max-width:600px){.lang-btn,.ibo-btn{font-size:11px;padding:5px 8px}}
  ...
  .cta-btn.locked{background:rgba(255,255,255,.14);color:rgba(255,255,255,.55);box-shadow:none;cursor:default}
  .prio-block{margin-top:18px;padding-top:16px;border-top:1px solid var(--gray-200)}
  .prio-title{font-size:11px;font-weight:700;letter-spacing:.06em;text-transform:uppercase;color:var(--green-mid);margin-bottom:10px}
  .prio-list{list-style:none;margin:0;padding:0;display:flex;flex-direction:column;gap:10px}
  .prio-item{display:flex;gap:11px;align-items:flex-start;font-size:14px;color:var(--gray-800,#1f2937)}
  .prio-n{flex:none;width:22px;height:22px;border-radius:50%;background:var(--green-light);color:var(--green);font-size:12px;font-weight:700;display:flex;align-items:center;justify-content:center;margin-top:1px}
  .prio-words{display:block;font-size:13px;font-weight:400;font-style:italic;color:var(--gray-500);margin-top:2px}
  .cta-lock-note{font-size:13px;color:rgba(255,255,255,.7);margin-top:12px;line-height:1.55}
  .ibo-bar{position:sticky;top:0;z-index:250;background:#1f2937;color:#fff;font-size:13px;padding:10px 16px;
    display:flex;flex-wrap:wrap;align-items:center;gap:8px 16px}
  .ibo-bar b{font-size:11px;letter-spacing:.08em;text-transform:uppercase;color:#9fd3b4}
  .ibo-sw{display:inline-flex;align-items:center;gap:9px;background:rgba(255,255,255,.08);border:1px solid rgba(255,255,255,.16);
    color:#fff;border-radius:999px;padding:5px 13px 5px 6px;cursor:pointer;font:inherit;font-size:12.5px}
  .ibo-sw:hover{background:rgba(255,255,255,.16)}
  .ibo-sw i{display:inline-block;flex:none;width:30px;height:18px;border-radius:9px;background:#6b7280;position:relative;transition:background .15s}
  .ibo-sw i::after{content:'';position:absolute;top:2px;left:2px;width:14px;height:14px;border-radius:50%;background:#fff;transition:transform .15s}
  .ibo-sw.on i{background:#34a56f}.ibo-sw.on i::after{transform:translateX(12px)}
  .ibo-hint{color:rgba(255,255,255,.55);font-size:12px}
  .ibo-all{color:#9fd3b4;font-size:12.5px;text-decoration:none;border:1px solid rgba(159,211,180,.4);border-radius:999px;padding:5px 12px}
  .ibo-all:hover{background:rgba(159,211,180,.15)}
  .ibo-out{margin-left:auto;background:none;border:0;color:rgba(255,255,255,.75);cursor:pointer;font:inherit;font-size:12.5px;text-decoration:underline}
  .sk{height:12px;border-radius:6px;background:#dfe4ea;margin:12px 0}.sk-h{width:38%;height:16px;margin-bottom:18px}
  .gate-blur{filter:blur(7px);opacity:.8;pointer-events:none;user-select:none}
  .gate-veil{position:fixed;inset:0;background:rgba(244,246,249,.4);z-index:200}
  .gate-card{position:fixed;left:50%;top:13vh;transform:translateX(-50%);width:min(440px,92vw);background:#fff;border-radius:20px;
    padding:30px 28px 26px;box-shadow:0 24px 70px rgba(20,25,34,.3);z-index:260;text-align:center}
  .gate-lock{width:54px;height:54px;border-radius:50%;background:var(--green-light);color:var(--green);display:flex;align-items:center;justify-content:center;margin:0 auto 14px}
  .gate-title{font-size:20px;font-weight:700;margin-bottom:8px;color:var(--gray-800,#1f2937)}
  .gate-body{font-size:14px;line-height:1.65;color:var(--gray-600,#55605A);margin-bottom:20px}
  .gate-btn{display:inline-block;background:var(--navy);color:#fff;border:0;border-radius:999px;padding:12px 26px;font:inherit;font-weight:600;font-size:14px;cursor:pointer}
  .gate-btn:disabled{opacity:.6;cursor:default}
  .gate-msg{font-size:12.5px;color:#b45309;margin-top:12px;min-height:16px}
  .ibo-modal{display:none;position:fixed;inset:0;z-index:500;background:rgba(20,25,34,.45);align-items:flex-start;justify-content:center;padding-top:14vh}
  .ibo-modal.open{display:flex}
  .ibo-dialog{width:min(360px,92vw);background:#fff;border-radius:18px;padding:24px 24px 20px;box-shadow:0 24px 70px rgba(20,25,34,.35)}
  .ibo-d-title{font-size:17px;font-weight:700;margin-bottom:14px;color:var(--gray-800,#1f2937)}
  .ibo-dialog label{display:block;font-size:12.5px;color:var(--gray-500,#6b7280);margin-bottom:6px}
  .ibo-dialog input{width:100%;box-sizing:border-box;font:inherit;font-size:18px;letter-spacing:.2em;padding:11px 14px;border:1.5px solid #d1d5db;border-radius:10px;outline:none}
  .ibo-dialog input:focus{border-color:var(--navy)}
  .ibo-err{min-height:18px;margin:8px 0 6px;font-size:12.5px;color:#b3261e}
  .ibo-actions{display:flex;justify-content:flex-end;gap:10px}
  .ibo-cancel{background:none;border:0;font:inherit;font-size:14px;color:var(--gray-500,#6b7280);cursor:pointer;padding:12px 14px}
...
${ibo ? `<div class="ibo-bar">
  <b>${T.iboBar}</b>
  <button class="ibo-sw${reportOpen ? ' on' : ''}" role="switch" aria-checked="${reportOpen}" onclick="iboSet('report', ${!reportOpen})"><i></i>${T.iboReport}: ${reportOpen ? T.stOpen : T.stLocked}</button>
  <button class="ibo-sw${unlocked ? ' on' : ''}" role="switch" aria-checked="${unlocked}" onclick="iboSet('products', ${!unlocked})"><i></i>${T.iboProducts}: ${unlocked ? T.stOpen : T.stLocked}</button>
  <span class="ibo-hint">${T.iboHint}</span>
  <a class="ibo-all" href="https://johnnnyay.github.io/product-marketing/admin.html" target="_blank" rel="noopener">${T.iboAll}</a>
  <button class="ibo-out" onclick="iboOut()">${T.iboSignOut}</button>
</div>` : ''}

<div class="hero">
  <div class="hero-tools">
  ${ibo ? '' : `<button class="ibo-btn" id="ibo-btn" onclick="iboOpen()" title="${T.iboOpen}" aria-label="${T.iboOpen}">···</button>`}
...
<div class="page-body">
  ...
  ${(r.products && r.products.length && r.calcUrl) ? ((unlocked || ibo) ? `
  <div class="cta-section">
    ...
    <div class="cta-passcode">${unlocked ? T.ctaPasscode : `${T.iboProducts}: ${T.stLocked} (${T.iboHint.replace(/\.$/, '')})`}</div>
  </div>` : `
  <div class="cta-section">
    <div class="cta-title">${T.ctaTitle}</div>
    <div class="cta-sub">${T.ctaSub}</div>
    <span class="cta-btn locked" aria-disabled="true">🔒 ${T.ctaLockedBtn}</span>
    <div class="cta-lock-note">${T.ctaLockedSub}</div>
  </div>`) : ''}
...
${gate ? `<div class="gate-veil"></div>
<div class="gate-card" role="dialog" aria-modal="true" aria-labelledby="gate-title">
  <div class="gate-lock"><svg width="26" height="26" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="4" y="11" width="16" height="10" rx="2"/><path d="M8 11V7a4 4 0 0 1 8 0v4"/></svg></div>
  <div class="gate-title" id="gate-title">${T.gateTitle}</div>
  <div class="gate-body">${T.gateBody}</div>
  <button class="gate-btn" id="gate-btn" onclick="gateCheck()">${T.gateCheck}</button>
  <div class="gate-msg" id="gate-msg" aria-live="polite"></div>
</div>` : ''}
<div class="ibo-modal" id="ibo-modal" onclick="if(event.target===this)iboClose()">
  <form class="ibo-dialog" onsubmit="return iboLogin(event)">
    <div class="ibo-d-title">${T.iboOpen}</div>
    <label for="ibo-code">${T.iboCodeLabel}</label>
    <input id="ibo-code" type="password" inputmode="numeric" autocomplete="off" maxlength="12">
    <div class="ibo-err" id="ibo-err" role="alert"></div>
    <div class="ibo-actions"><button type="button" class="ibo-cancel" onclick="iboClose()">${T.iboCancel}</button><button type="submit" class="gate-btn" id="ibo-go">${T.iboSignIn}</button></div>
  </form>
</div>
<script>
...
/* Partner sign in and the two locks. The passcode is checked on the server, which sets a session
   cookie; every change goes through /api/unlock. */
var RID = ${JSON.stringify(rid)};
var TX = ${JSON.stringify({ wrong: T.iboWrong, fail: T.iboFail, check: T.gateCheck, checking: T.gateChecking, still: T.gateStill })};
function byId(i){ return document.getElementById(i); }
function post(body){
  return fetch('/api/unlock', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) });
}
function iboOpen(){
  var m = byId('ibo-modal'); m.classList.add('open');
  byId('ibo-err').textContent = ''; var i = byId('ibo-code'); i.value = '';
  setTimeout(function(){ i.focus(); }, 40);
}
function iboClose(){ byId('ibo-modal').classList.remove('open'); }
document.addEventListener('keydown', function(e){ if (e.key === 'Escape') iboClose(); });
function iboLogin(ev){
  if (ev) ev.preventDefault();
  var code = byId('ibo-code').value.trim(); if (!code) return false;
  var go = byId('ibo-go'); go.disabled = true;
  post({ action: 'login', code: code }).then(function(r){
    if (r.ok) { location.reload(); return; }
    go.disabled = false; byId('ibo-err').textContent = r.status === 401 ? TX.wrong : TX.fail;
  }).catch(function(){ go.disabled = false; byId('ibo-err').textContent = TX.fail; });
  return false;
}
function iboSet(field, value){
  var body = { action: 'set', rid: RID }; body[field] = value;
  post(body).then(function(){ location.reload(); }).catch(function(){ alert(TX.fail); });
}
function iboOut(){ post({ action: 'logout' }).then(function(){ location.reload(); }); }
function reportIsOpen(){
  return fetch('/api/unlock?r=' + RID, { cache: 'no-store' }).then(function(r){ return r.json(); }).then(function(j){ return !!j.report; });
}
function gateCheck(){
  var b = byId('gate-btn'), m = byId('gate-msg'); if (!b) return;
  b.disabled = true; b.textContent = TX.checking; m.textContent = '';
  reportIsOpen().then(function(open){
    if (open) { location.reload(); return; }
    b.disabled = false; b.textContent = TX.check; m.textContent = TX.still;
  }).catch(function(){ b.disabled = false; b.textContent = TX.check; m.textContent = TX.fail; });
}
${gate ? `/* A locked page opens itself the moment a partner unlocks it: check every 20 seconds while it is
   on screen, for up to 30 minutes. */
(function(){
  var n = 0;
  var t = setInterval(function(){
    if (document.hidden) return;
    if (++n > 90) { clearInterval(t); return; }
    reportIsOpen().then(function(open){ if (open) location.reload(); }).catch(function(){});
  }, 20000);
})();` : ''}
</script>
</body>
</html>`;
  if (gate) html = html.replace(/<div class="page-body">[\s\S]*?(?=\s*<div class="report-footer">)/, () => `<div class="page-body gate-blur">${skeleton}`);
  return html;
}
