...
const { CHROME, STAGES, LOCALES, LOCALE_NAME, CANONICAL, stageKey } = require('./i18n');
...
<style>
  ...
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  .lang-toggles{position:absolute;top:14px;right:18px;display:flex;gap:6px;flex-wrap:wrap;justify-content:flex-end}
  .lang-toggle{font-size:11.5px;font-weight:600;letter-spacing:.04em;
    color:rgba(255,255,255,.92);background:rgba(255,255,255,.16);border:1px solid rgba(255,255,255,.28);
    border-radius:999px;padding:5px 13px;text-decoration:none;backdrop-filter:blur(4px);white-space:nowrap}
...
<div class="hero">
  ${others.length ? `<div class="lang-toggles">${others.map(l => `<a class="lang-toggle" href="?r=${filename.replace(/\.[^.]+$/, '')}${l === CANONICAL ? '' : `&lang=${l}`}">${LOCALE_NAME[l]}</a>`).join('')}</div>` : ''}
