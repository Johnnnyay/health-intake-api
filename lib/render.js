...
function buildHTML(r, form, filename, assessmentDate, locale, available) {
  ...
  const productCard = ({ p, i }) => {
    ...
    /* Dose detail sits behind a disclosure rather than in its own card. It was two
       separate sections repeating what the stage already said, and it forced lines
       like "From food: not applicable" onto items where food is not the point. */
    const detail = (d.servingsPerDay || d.food || n.length) ? `
      <details class="dose-detail">
        <summary>${T.detailBtn}</summary>
        <div class="dose-detail-body">
          ${d.servingsPerDay ? `<div class="dose-line"><b>${T.dHow}</b> ${d.servingsPerDay} ${d.unit || ''} ${T.perDay}${
             d.bottlesPerMonth ? ` &nbsp;·&nbsp; ${T.monthly} ${d.bottlesPerMonth}` : ''}</div>` : ''}
          ${d.when ? `<div class="dose-line"><b>${T.dWhen}</b> ${d.when}</div>` : ''}
          ${d.why ? `<div class="dose-line"><b>${T.dWhy}</b> ${d.why}</div>` : ''}
          ${d.food ? `<div class="dose-food"><b>${T.foodInstead}</b> ${d.food}</div>` : ''}
          ...
          ${n.some(x => x.source) ? `<div class="dose-src">${T.dSource}: ${
    ...
    <div class="product-card">
      <div class="product-head">
        ...
        ${d.servingsPerDay ? `<span class="product-dose">${d.servingsPerDay} ${d.unit || ''}/${T.perDay}</span>` : ''}
      </div>
      <div class="product-rationale">${p.rationale}</div>
      ${p.note ? `<div class="product-note"><span class="note-label">${T.takeWith}</span>${p.note}</div>` : ''}
      ${detail}
    </div>`;
  };
...
<style>
  ...
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  .dose-detail-body{padding-top:6px}
  ...
  .product-name{color:var(--navy);font-size:14px;font-weight:600;color:var(--amber);margin-bottom:4px}
